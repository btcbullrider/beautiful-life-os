...
const CL = [
  ...
  { id: "callmom", label: "Call Mom", emoji: "📞", cat: "morning", desc: "A daily touchpoint to express love and gratitude. Honoring your family aligns with giving first and staying grounded." },
  ...
  { id: "heartcoherence", label: "Heart Coherence", emoji: "❤️", cat: "morning", desc: "HeartMath's coherence technique: place your attention on your heart, breathe slowly (5 sec in, 5 sec out), and intentionally feel a positive emotion — gratitude, love, peace. This synchronises heart and brain rhythms, lowering cortisol and opening you to intuition and higher-order thinking." },
  { id: "exercise", label: "Move Your Body", emoji: "💪", cat: "evening", desc: "Non-negotiable energy infrastructure. Adams is clear: manage energy, not time. A strong body supports a clear mind, an open heart, and the capacity to serve. A depleted vessel pours nothing." },
...
export default function App() {
  ...
  useEffect(() => {
    (async () => {
      ...
      const SK = getSK();
      const [c, a, j, p, w, ji, hi, ord] = await Promise.all([ld(SK.cl, {}), ld(SK.af, DEFAULT_AFF), ld(SK.jo, ""), ld(SK.wr, {}), ld(SK.wn, ""), ld(SK.ji, []), ld(SK.hi, {}), ld(SK.ord, null)]);
      ...
      setAff(a); setPs(p); setWn(w); setJoIdx(localJoIdx); setHistory(hi);
      if (ord) {
        ...
        let finalOrder = [...cleaned];
        if (missing.includes('callmom')) {
          const idx = finalOrder.indexOf('scripture');
          if (idx !== -1) {
            finalOrder.splice(idx + 1, 0, 'callmom');
            missing.splice(missing.indexOf('callmom'), 1);
          }
        }
        setOrder([...finalOrder, ...missing]);
      }
      setLoaded(true);
    })();
  }, []);
      ...
      <div style={{ maxWidth: 760, margin: "0 auto", padding: "0 1.5rem" }}>
        <header style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "1.5rem 0 1rem", borderBottom: "1px solid rgba(200,169,81,0.1)" }}>
          ...
          <div style={{ display: "flex", flexDirection: "column", alignItems: "center", background: "rgba(200,169,81,0.06)", border: "1px solid rgba(200,169,81,0.15)", borderRadius: 4, padding: "0.4rem 0.9rem" }}>
            <span style={{ fontSize: "1.3rem", fontWeight: 600, color: "#C8A951" }}>{currentStreak}</span>
        ...
        <main style={{ padding: "1.5rem 0 3rem" }}>
          ...
          {tab === "tracker" && <TrackerTab history={history} currentStreak={currentStreak} />}
